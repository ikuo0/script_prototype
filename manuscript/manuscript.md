...
演算はレジスタ間計算として実装し、レジスタ概念への変数コピー命令も実装すれば良いでしょう、アドレス直接参照をせずとも変数シンボル、配列インデックス、辞書型のキーなど、変数アクセスの手段は豊富です。<br>
if、forの分岐やループはプログラムカウンタ（１次元配列のインデックス）を操作することで再現できそうです。
